...
public class GPSPoint {
	...
	public GPSPoint(GPSPoint homeLocation) {
		if (homeLocation == null) {
			throw new NullPointerException("Can't copy a null GPSPoint");
		}
	...
	/**
	 * @return the <i>approximate</i> distance (using the spherical law of
	 *         cosines) of 'this' GPSPoint to 'other' in kilometers.
	 **/
	public double distanceTo(GPSPoint other) {
		...
		double cosVal = Math.cos(firstLatRads) * Math.cos(secondLatRads)
				* Math.cos(firstLongRads - secondLongRads);
		double sinVal = Math.sin(firstLatRads) * Math.sin(secondLatRads);

		if (Math.abs((cosVal + sinVal) - 1.0) <= EPSILON) {
			return 0.0;
		} // prevents NaN on acos(1+).
